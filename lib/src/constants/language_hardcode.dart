import 'package:get/get.dart';

class LocalString extends Translations {
  @override
  // TODO: implement keys
  Map<String, Map<String, String>> get keys => {
        'en_US': {
          'no new messages':
              'no new messages to show\n please press refresh...',
          'Chat now': 'Chat now',
          'Recents': 'Recents',
          'Alerts': 'Alerts',
          'Unresolved Phone Numbers': 'Unresolved Phone Numbers',
          'Unresolved messages': 'Unresolved messages',
          'Unresolved Emails': 'Unresolved Emails',
          'Call Logs': 'Call Logs',
          'language': 'Please Choose a language',
          'confirm': 'Confirm',
          'home': 'Home',
          'calls': 'Calls',
          'messages': 'Messages',
          'Login/Sign Up': 'Login/Sign Up',
          'Enter your Phone Number': 'Enter your Phone Number\n',
          'Phone Number': 'Phone Number',
          'App_Permissions': 'App Permissions',
          'clauses':
              'We understand the nature and sensitivity of this topic and have taken strong measures to ensure that your data is not compromised.',
          'T&C': 'Terms And Conditions',
          'I agree to the terms and conditions':
              'I agree to the terms and conditions',
          'I Agree': 'I Agree',
          'Registration Page': 'Registration Page',
          'Almost There !': 'Almost There !',
          'First Name': 'First Name',
          'Last Name': 'Last Name',
          'Email ID (optional)': 'Email ID (optional)',
          'Accept the agreements': 'Accept the agreements',
          'Create an Account': 'Create an Account'
        },
        'hi_IN': {
          'no new messages': 'नए संदेश नहीं\n रिफ्रेश दबाएं...',
          'Chat now': 'अभी चैट करें',
          'Recents': 'हाल के',
          'Alerts': 'चेतावनियाँ',
          'Unresolved Phone Numbers': 'अनसुलझे फोन नंबर',
          'Unresolved messages': 'अनसुलझे संदेश',
          'Unresolved Emails': 'अनसुलझे ईमेल',
          'Call Logs': 'कॉल लॉग्स',
          'language': 'कृपया भाषा चुनें',
          'confirm': 'पुष्टि करें',
          'home': 'होम',
          'calls': 'कॉल',
          'messages': 'संदेश',
          'Login/Sign Up': 'लॉग इन/साइन इन',
          'Enter your Phone Number': 'अपना फोन नंबर डालें',
          'Phone Number': 'फ़ोन नंबर',
          'App_Permissions': 'ऐप अनुमतियाँ',
          'T&C': 'नियम और शर्तें',
          'clauses': 'आपकी गोपनीयता हमारी प्राथमिकता है। हम केवल आवश्यक डेटा को एकत्रित करते हैं, और आपका व्यक्तिगत डेटा सुरक्षित रहता है!\n\n'
              'हम कैसे काम करते हैं:\n'
              'निश्चल फ़ॉक्सइंटेलिजेंस की सेवा है। ऐप को मुफ़्त बनाने के लिए, हम इलेक्ट्रॉनिक वाणिज्य पर सांख्यिकीय उद्देश्यों के लिए बाज़ार डेटा का प्रचार करते हैं। हम किसी भी प्रकार के विज्ञापन, लक्ष्यण, डेटाबेस, या प्रोफ़ाइल वृद्धि के लिए कोई डेटा प्रसारित नहीं करते हैं।\n\n'
              'आपका डेटा सुरक्षित रहता है:\n'
              'निश्चल का उपयोग करके, आपको कभी भी विज्ञापनों के लिए लक्षित नहीं किया जाएगा, चाहे हमारे उत्पाद पर हो या बाद में।',
          'I agree to the terms and conditions':
              'मैं नियम और शर्तों से सहमत हूँ।',
          'I Agree': 'मैं सहमत हूँ',
          'Registration Page': 'पंजीकरण पेज',
          'Almost There !': 'अब तो बस थोड़ी दूर है !',
          'First Name': 'पहला नाम',
          'Last Name': 'अंतिम नाम',
          'Email ID (optional)': 'ईमेल आईडी (वैकल्पिक)',
          'Accept the agreements': 'समझौतों को स्वीकार करें',
          'Create an Account': 'खाता बनाएं'
        },
        'gu_IN': {
          'no new messages': 'કોઈ નવા સંદેશો નથી\n રિફ્રેશ દબાવો...',
          'Chat now': 'હવે ચેટ કરો',
          'Recents': 'તાજેતરના',
          'Alerts': 'ચેતવણીઓ',
          'Unresolved Phone Numbers': 'અનિરાકરણ ફોન નંબરો',
          'Unresolved messages': 'અનિરાકરણ સંદેશો',
          'Unresolved Emails': 'અનિરાકરણ ઇમેઇલ્સ',
          'Call Logs': 'કૉલ લોગ્સ',
          'language': 'કૃપા કરીને એક ભાષા પસંદ કરો',
          'confirm': 'પુષ્ટિ કરો',
          'home': 'હોમ',
          'calls': 'કૉલ્સ',
          'messages': 'સંદેશો',
          'Login/Sign Up': 'લૉગ ઇન/સાઇન ઇન',
          'Enter your Phone Number': 'તમારો ફોન નંબર દાખલ કરો',
          'Phone Number': 'ફોન નંબર',
          'App_Permissions': 'એપ્લિકેશન પરવાનગીઓ',
          'T&C': 'શરતો અને શરતો',
          'clauses': 'તમારી ગોપનીયતા અમારી પ્રાથમિકતા છે. અમે માત્ર આવશ્યક ડેટા એકત્ર કરીએ છીએ, અને તમારા વ્યક્તિગત ડેટા સંરક્ષિત છે!\n\n'
              'અમે કેવી રીતે કામ કરીએ:\n'
              'નિશ્ચલ એ ફોક્સઇન્ટેલિજેન્સની સેવા છે. ઍપને મુફ્ત બનાવવા માટે, અમે ઇ-વાણિજ્યિક પરિસર પર આંકડાશાસ્ત્રીય ઉદ્દેશ્યો માટે માર્કેટિંગ ડેટાનો ઉપયોગ કર્યો છે. અમે કોઈ પણ પ્રકારની વિજ્ઞાપન, લક્ષ્યિત, ડેટાબેસ, અથવા પ્રોફાઇલ વૃદ્ધિ માટે કોઈ ડેટા પ્રસારિત નહીં કરીએ છીએ.\n\n'
              'તમારા ડેટા સંરક્ષિત છે:\n'
              'નિશ્ચલનો ઉપયોગ કરીને, તમને ક્યારેયપણ પ્રચાર માટે લક્ષ્યિત નહીં કરવામાં આવશે, આમાં તમારા ઉત્પાદ પર નહીં અને તત્કાલિક પરંતુ',
          'I agree to the terms and conditions':
              'હું શરતો અને શરતોને સ્વીકારું છું',
          'I Agree': 'હું સહમત છું',
          'Registration Page': 'રજીસ્ટ્રેશન પેજ',
          'Almost There !': 'માત્ર થોડી દૂર છે !',
          'First Name': 'પ્રથમ નામ',
          'Last Name': 'છેલ્લું નામ',
          'Email ID (optional)': 'ઇમેઇલ આઈડી (વૈકલ્પિક)',
          'Accept the agreements': 'સમઝોતાઓને સ્વીકારો',
          'Create an Account': 'એક ખાતું બનાવો'
        },
        'kn_IN': {
          'no new messages': 'ಹೊಸ ಸಂದೇಶಗಳಿಲ್ಲ\n ದಯವಿಟ್ಟು ರಿಫ್ರೆಶ್ ಒತ್ತಿ...',
          'Chat now': 'ಈಗ ಚಾಟ್ ಮಾಡಿ',
          'Recents': 'ಇತ್ತೀಚಿನದು',
          'Alerts': 'ಎಚ್ಚರಿಕೆಗಳು',
          'Unresolved Phone Numbers': 'ಅನಿರ್ಧಾರಿತ ಫೋನ್ ಸಂಖ್ಯೆಗಳು',
          'Unresolved messages': 'ಅನಿರ್ಧಾರಿತ ಸಂದೇಶಗಳು',
          'Unresolved Emails': 'ಅನಿರ್ಧಾರಿತ ಇಮೇಲ್ಗಳು',
          'Call Logs': 'ಕಾಲ್ ಲಾಗ್ಗಳು',
          'language': 'ದಯವಿಟ್ಟು ಒಂದು ಭಾಷೆಯನ್ನು ಆಯ್ಕೆ ಮಾಡಿ',
          'confirm': 'ದೃಢೀಕರಿಸಿ',
          'home': 'ಮುಖಪುಟ',
          'calls': 'ಕಾಲ್ಸ್',
          'messages': 'ಸಂದೇಶಗಳು',
          'Login/Sign Up': 'ಲಾಗಿನ್/ಸೈನ್ ಇನ್',
          'Enter your Phone Number': 'ನಿಮ್ಮ ಫೋನ್ ಸಂಖ್ಯೆಯನ್ನು ನಮೂದಿಸಿ',
          'Phone Number': 'ದೂರವಾಣಿ ಸಂಖ್ಯೆ',
          'App_Permissions': 'ಅಪ್ಲಿಕೇಶನ್ ಅನುಮತಿಗಳು',
          'T&C': 'ನಿಯಮಗಳು ಮತ್ತು ಶರತ್ತುಗಳು',
          'clauses':
              'ನಿಮ್ಮ ಗೌಪ್ಯತೆಯು ನಮ್ಮ ಪ್ರಾಧಾನ್ಯತೆಯಾಗಿದೆ. ನಾವು ಮಾತ್ರ ಅಗತ್ಯವಿದ್ದ ಡೇಟಾವನ್ನು ಸಂಗ್ರಹಿಸುತ್ತೇವೆ, ಮತ್ತು ನಿಮ್ಮ ವೈಯಕ್ತಿಕ ಡೇಟಾ ಸಂರಕ್ಷಿತವಾಗಿದೆ!\n\n'
                  'ನಾವು ಹೇಗೆ ಕೆಲಸ ಮಾಡುತ್ತಿದ್ದೇವೆ:\n'
                  'ನಿಶ್ಚಲ ಫಾಕ್ಸಿಂಟೆಲಿಜೆನ್ಸ್ ಸೇವೆಯೇ ನಾವು ಒದಗಿಸುವ ಆಪ್ಲಿಕೇಶನ್ ಆಗಿದೆ. ಆಪ್ಲಿಕೇಶನ್ ಉಚಿತವಾಗಿದ್ದು, ನಾವು ಇ-ವಾಣಿಜ್ಯದ ಪ್ರಾಸಂಗಿಕ ಉದ್ದೇಶಗಾಗಿ ಪರಿಸರದ ಡೇಟಾವನ್',
          'I agree to the terms and conditions':
              'ನಾನು ನಿಯಮಗಳನ್ನು ಮತ್ತು ಶರತ್ತುಗಳನ್ನು ಒಪ್ಪಿಕೊಳ್ಳುತ್ತೇನೆ.',
          'I Agree': 'ನಾನು ಒಪ್ಪಿಕೊಳ್ಳುತ್ತೇನೆ',
          'Registration Page': 'ನೋಂದಾಯಿಸುವ ಪುಟ',
          'Almost There !': 'ಅಲ್ಲಿ ಸ್ವಲ್ಪ ಮಾತ್ರ !',
          'First Name': 'ಮೊದಲ ಹೆಸರು',
          'Last Name': 'ಕೊನೆಯ ಹೆಸರು',
          'Email ID (optional)': 'ಇಮೇಲ್ ಐಡಿ (ಐಚ್ಛಿಕ)',
          'Accept the agreements': 'ಒಪ್ಪಿಗೆ ಮಾಡಿಕೊಳ್ಳಿ',
          'Create an Account': 'ಖಾತೆ ತೆರೆಯಿರಿ'
        },
      };
}
